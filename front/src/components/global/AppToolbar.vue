<template>
  <v-toolbar app clipped-left class="app-toolbar glow-z1">
    ...
    <v-toolbar-title>
      <v-breadcrumbs :items="breadItems">
        <template v-slot:item="props">
          <span v-html="props.item.name" :class="!props.item.isLast ? 'font-weight-light onlight--text text--darken-4' : 'font-weight-medium'" />
        </template>
    ...
    <v-toolbar-items color="transparent">
      <ToolbarItems v-for="(route, indx) in routes" :key="indx" :item="route" :isActive="breadItems.some(item => item.name.toLowerCase() === route.name.toLowerCase())" />
      <div class="tw-w-full tw-flex flex tw-items-center" :style="{ height: `${topbar}px`}">
        <v-menu :nudge-width="100" offset-y transition="slide-x-reverse-transition">
          <template v-slot:activator="{ on }">
            <v-toolbar-title v-on="on" class="tw-cursor-pointer">
              <v-avatar size="48">
                <svg class="tw-h-8 tw-w-auto tw-text-primary-light tw-fill-current" version="1.1" viewBox="0 0 800 800" enable-background="new 0 0 800 800" xml:space="preserve">
...
export default {
  ...
  methods: {
    routeHandler (newVal, oldVal) {
      const bcItems = []
      const itemCt = newVal.matched.length - 1
      newVal.matched.forEach((match, idx) => {
        bcItems.push({
          name: match.name,
          isLast: idx === itemCt
        })
      })
      this.breadItems = bcItems
    }
  }
}
</script>
